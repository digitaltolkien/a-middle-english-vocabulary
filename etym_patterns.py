import re

languages = [  # without final period
    "AFr",  # Anglo-French
    "Anglo-L",  # Anglo-Latin (?)
    "Du",  # Dutch
    "E",  # English
    "Mn.E",  # Modern English
    "Fr",  # French
    "Fris",  # (Modern) Frisian (dialects)
    "G",  # German
    "Goth",  # Gothic
    "Icel",  # (Modern) Icelandic
    "Kt",  # OKt. Kentish; Kentish dialect of Old English.
    "L",  # Latin
    "Med.L",  # Mediaeval Latin
    "LG",  # ???
    "MDu",  # Middle Dutch
    "ME",  # Middle English
    "MHG",  # Middle High German
    "MLG",  # Middle Low German
    "Norw",  # Norwegian (not in list)
    "Nth",  # Northumbrian;
    "ONth",  # Northumbrian dialect of Old English.
    "NWM",  # North West Midland.
    "OE",  # Old English
    "OFr",  # Old French
    "OFris",  # Old Frisian
    "OHG",  # Old High German
    "OIcel",  # Old Icelandic (?)
    "OIr",  # Old Irish
    "ON",  # Old NOrse
    "ONFr",  # Northern dialects of Old French
    "OS",  # Old Saxon (Old Low German)
    "OSwed",  # Old Swedish (?)
    "Swed",  # Swedish
    # WS.; OWS. West Saxon (dialect of Old English).
]

IT = r"_[^_]+_"
LANG = fr"(({'|'.join(languages)})\.|Latin|Welsh)"
CAP_WORD = r"[A-ZÞ]([a-zþȝ]|p\(p\))+(\([en]\))?"
GRM = r"(prep|n|v|adj|adv|pl|fem|neut|masc|str|wk|dat|acc|gen|sg|nom|pa|t|3 sg|conj|intr|compar|collect|refl|pret|pres|subj|p)\."
SUP = r"\^\d+"
QV = r"_q\.v\._"
NED = r"_N\.E\.D\._"  # The Oxford (New) English Dictionary
GLOSS = r"([a-z\- ']+|Briton)"
ETC = r"&c\."
INFL_BY = r"infl\. by"
REF = "VII 167"

GRMS = fr"{GRM}( {GRM})*"

regexes = [
    re.compile(r)
    for r in [
        fr"_Cf\._ {LANG} {IT}, {GLOSS}, {GLOSS}\.$",  # italics inconsistency
        fr"_See_ next\.$",
        fr"{CAP_WORD} \+ {CAP_WORD}, _{GRM}_{SUP}$",
        fr"{CAP_WORD} \+ {CAP_WORD}, _{GRM}_$",
        fr"{CAP_WORD} \+ {CAP_WORD}, {QV}$",
        fr"{CAP_WORD}- \+ {CAP_WORD}, _{GRM}_{SUP}$",
        fr"{CAP_WORD}- \+ {CAP_WORD}, {QV}$",
        fr"{CAP_WORD}, _{GRM}_ \+ {CAP_WORD}, {QV}$",
        fr"{CAP_WORD}, _{GRM}_ \+ {CAP_WORD}\.$",
        fr"{CAP_WORD}, _{GRM}_ \+ {LANG} {IT} from {CAP_WORD}\.$",
        fr"{CAP_WORD}, {CAP_WORD} {INFL_BY} {CAP_WORD}\.$",
        fr"{CAP_WORD}, {CAP_WORD} \+ {CAP_WORD}, {QV}$",
        fr"{IT} \+ {CAP_WORD}; cf\. {IT}, s\.v\. {CAP_WORD}, _{GRM}_$",
        fr"{IT} \+ {LANG} {IT}, {GLOSS}\.$",
        fr"{IT} \+ {LANG} {IT}, {IT}\.$",
        fr"{IT} \+ {LANG} {IT}; cf\. {IT}, {GLOSS}\.$",
        fr"{IT} \+ {LANG} {IT}\.$",
        fr"{LANG} {IT} {GLOSS}\.$",
        fr"{LANG} {IT} {GRM}, {GLOSS}\.$",
        fr"{LANG} {IT} {GRMS}$",
        fr"{LANG} {IT} \({GLOSS}\) {IT} \({IT}\)\.$",
        fr"{LANG} {IT} \({GRM} {IT}, {IT}\); {LANG} {IT}.$",
        fr"{LANG} {IT} \({GRM}\), {GLOSS}, {GLOSS}; _cf._ {CAP_WORD}, _{GRM}_{SUP}$",
        fr"{LANG} {IT} \({GRM}\); _see_ Appendix, p\. \d+\.$",
        fr"{LANG} {IT} \({IT}, {GRM}\); {IT}\.$",
        fr"{LANG} {IT} \({IT}, {GRMS}\); cf\. next\.$",
        fr"{LANG} {IT} \({IT}\) \+ {IT} \(Kt\. {IT}\)\.$",
        fr"{LANG} {IT} \({IT}\) \+ {LANG} {IT}\.$",
        fr"{LANG} {IT} \({IT}\), {IT}, {ETC}; {LANG} {GRM} {IT}, {ETC}$",
        fr"{LANG} {IT} \({IT}\), {IT}, {IT}.",
        fr"{LANG} {IT} \({IT}\), or {IT}\.$",
        fr"{LANG} {IT} \({IT}\); {IT}, {IT}\.$",
        fr"{LANG} {IT} \({IT}\)\.$",
        fr"{LANG} {IT} \({LANG} {IT}, {LANG} {IT}\)\.$",
        fr"{LANG} {IT} \({LANG} {IT}\) infl\. by {IT}\.$",
        fr"{LANG} {IT} \({LANG} {IT}\), or {LANG} {IT}\.$",
        fr"{LANG} {IT} \({LANG} {IT}\); _see_ Appendix p\. 280.$",  # appendix not abbreviated
        fr"{LANG} {IT} \(\? late {LANG} {IT}\); but see {CAP_WORD}, _{GRM}_$",
        fr"{LANG} {IT} \(a land-measure\); {LANG} {IT}\.$",
        fr"{LANG} {IT} \(cf\. {LANG} {IT}, {LANG} {IT}\)\.$",
        fr"{LANG} {IT} \(from {IT}\) \+ {IT}\.$",
        fr"{LANG} {IT} \(in Sweet\)\.$",
        fr"{LANG} {IT} \(late {IT}\), {ETC}$",
        fr"{LANG} {IT} \(once\), {IT}\.$",
        fr"{LANG} {IT} \(or {IT}\) {IT}\.$",
        fr"{LANG} {IT} \+ {CAP_WORD}, {QV}$",
        fr"{LANG} {IT} \+ {CAP_WORD}\.$",
        fr"{LANG} {IT} \+ {GRM} {IT}\.$",
        fr"{LANG} {IT} \+ {IT} \(_see_ {CAP_WORD}\)\.$",
        fr"{LANG} {IT} \+ {IT} \(intensive\)\.$",
        fr"{LANG} {IT} \+ {IT} \(properly {GRM} with prec\. noun\).$",
        fr"{LANG} {IT} \+ {IT} indef\.$",
        fr"{LANG} {IT} \+ {IT}, {GLOSS} \(_see_ {CAP_WORD}\)\.",
        fr"{LANG} {IT} \+ {IT}, {GRMS}$",
        fr"{LANG} {IT} \+ {IT}, pp\. of {IT}\.$",
        fr"{LANG} {IT} \+ {IT}, pp\.$",
        fr"{LANG} {IT} \+ {IT}; cf\. {IT}, {ETC}$",
        fr"{LANG} {IT} \+ {IT}; cf\. {IT}, {GLOSS}\.$",
        fr"{LANG} {IT} \+ {IT}; cf\. {IT}\.$",
        fr"{LANG} {IT} \+ {IT}; cf\. {LANG} {IT}\.$",
        fr"{LANG} {IT} \+ {IT}; cf\. {LANG} \(late\) {IT}\.$",
        fr"{LANG} {IT} \+ {IT}; cf\. prec\.$",
        fr"{LANG} {IT} \+ {IT}\.$",
        fr"{LANG} {IT} \+ {LANG} {IT} \(cf. {LANG}, {LANG} {IT}\).$",
        fr"{LANG} {IT} \+ {LANG} {IT}, {LANG} {IT}\.$",
        fr"{LANG} {IT} \+ {LANG} {IT}; cf\. {LANG} {IT}\.$",
        fr"{LANG} {IT} \+ {LANG} {IT}\.$",
        fr"{LANG} {IT} \+ \? {IT} \? {IT}\.$",
        fr"{LANG} {IT} and {IT}\.$",
        fr"{LANG} {IT} and {LANG} {IT}\.$",
        fr"{LANG} {IT} blended with {IT} \({IT}\)\.$",
        fr"{LANG} {IT} from {LANG} {IT}\.$",
        fr"{LANG} {IT} infl\. by {IT}, {GRM}$",
        fr"{LANG} {IT} infl\. by senses of related {LANG} {IT}.$",
        fr"{LANG} {IT} or {IT}, infl\. by next\.$",
        fr"{LANG} {IT} rel\. to {IT}\.$",
        fr"{LANG} {IT} through {LANG}$",
        fr"{LANG} {IT} under influence of {IT}\.$",
        fr"{LANG} {IT} with substitution of {GRMS} {IT}.$",
        fr"{LANG} {IT}, {CAP_WORD}\.$",
        fr"{LANG} {IT}, {ETC}, {GLOSS}\.$",
        fr"{LANG} {IT}, {ETC}, pet-name assoc. with 'John'\.$",
        fr"{LANG} {IT}, {GLOSS} \({GLOSS}\)\.$",
        fr"{LANG} {IT}, {GLOSS} \(late {LANG} {GRM} {IT}\)\.$",
        fr"{LANG} {IT}, {GLOSS} \(or stem of prec\.\) \+ {IT}\.$",
        fr"{LANG} {IT}, {GLOSS} \(sound\)\.$",
        fr"{LANG} {IT}, {GLOSS} \+ {LANG} {IT}\.$",
        fr"{LANG} {IT}, {GLOSS} infl\. by unrelated {LANG} {IT}, {GLOSS}\.$",
        fr"{LANG} {IT}, {GLOSS}, {GLOSS} \(of arrow, spear, {ETC}\)\.$",
        fr"{LANG} {IT}, {GLOSS}, {GLOSS}\.$",
        fr"{LANG} {IT}, {GLOSS}, \(in verse\) {GLOSS}\.$",
        fr"{LANG} {IT}, {GLOSS}, from {LANG} {IT}\.$",
        fr"{LANG} {IT}, {GLOSS}; _cf\._ {CAP_WORD}\.$",
        fr"{LANG} {IT}, {GLOSS}; {GRMS} {IT}\.$",
        fr"{LANG} {IT}, {GLOSS}; {IT}, {GLOSS}\.$",
        fr"{LANG} {IT}, {GLOSS}; {LANG} {IT}, {GLOSS}, {GLOSS}\.$",
        fr"{LANG} {IT}, {GLOSS}; {LANG} {IT}, {GLOSS}\.$",
        fr"{LANG} {IT}, {GLOSS}; cf\. {LANG} {IT}\.$",
        fr"{LANG} {IT}, {GLOSS}; see {NED}$",
        fr"{LANG} {IT}, {GLOSS}\.$",
        fr"{LANG} {IT}, {GLOSS}\.$",  # inconsistent comma?
        fr"{LANG} {IT}, {GRM} {IT}, {IT}\.$",
        fr"{LANG} {IT}, {GRM} \+ {IT}\.$",
        fr"{LANG} {IT}, {GRM} and {GRM}; {IT}, {GRM}$",
        fr"{LANG} {IT}, {GRM} and {GRM}$",
        fr"{LANG} {IT}, {GRM} of {IT}\.$",
        fr"{LANG} {IT}, {GRM}, {GLOSS}; {LANG} {IT}, {GLOSS}\.$",
        fr"{LANG} {IT}, {GRM}, {GRM}; {IT}, {GRM}$",
        fr"{LANG} {IT}, {GRM}, {IT}, {GRM}$",
        fr"{LANG} {IT}, {GRM}, and {IT}, {GRM}$",
        fr"{LANG} {IT}, {GRM}; {IT} \({GRMS} {IT}\), {GRM}$",
        fr"{LANG} {IT}, {GRM}; {IT}, {GRM}; {IT}, {GRM}$",
        fr"{LANG} {IT}, {GRM}; {IT}\.$",
        fr"{LANG} {IT}, {GRM}; cf\. {IT}\.$",
        fr"{LANG} {IT}, {GRM}; cf\. {LANG} {IT}\.$",
        fr"{LANG} {IT}, {GRM}$",
        fr"{LANG} {IT}, {GRMS} {IT}\.$",
        fr"{LANG} {IT}, {GRMS}, {GLOSS}\.$",
        fr"{LANG} {IT}, {GRMS}, for \(by\) itself; separately\.$",
        fr"{LANG} {IT}, {GRMS}$",  # inconsistent comma?
        fr"{LANG} {IT}, {INFL_BY} {IT}\.$",
        fr"{LANG} {IT}, {INFL_BY} next\.$",
        fr"{LANG} {IT}, {INFL_BY} prec\.$",
        fr"{LANG} {IT}, {IT} \({GRM}\), {IT}\.$",
        fr"{LANG} {IT}, {IT} \({LANG} {IT}\) \+ {GRM} {IT}\.$",
        fr"{LANG} {IT}, {IT} \(fem\.\)\.$",
        fr"{LANG} {IT}, {IT} \(still {LANG}\); cf\. {IT}, {IT}\.$",
        fr"{LANG} {IT}, {IT} \+ {GRM} {IT}\.$",
        fr"{LANG} {IT}, {IT} \+ {IT}\.$",
        fr"{LANG} {IT}, {IT}, {ETC}, {GRM}$",
        fr"{LANG} {IT}, {IT}, {ETC}$",
        fr"{LANG} {IT}, {IT}, {GLOSS}, {ETC}$",
        fr"{LANG} {IT}, {IT}, {GLOSS}, {GLOSS}\.$",
        fr"{LANG} {IT}, {IT}, {GLOSS}\.$",
        fr"{LANG} {IT}, {IT}, {GRM}{SUP}$",
        fr"{LANG} {IT}, {IT}, {IT}, {ETC}$",
        fr"{LANG} {IT}, {IT}, {IT}, {IT}, {ETC}$",
        fr"{LANG} {IT}, {IT}, {IT}, {IT}\.$",
        fr"{LANG} {IT}, {IT}, {IT}; see {NED}$",
        fr"{LANG} {IT}, {IT}, {IT}\.$",
        fr"{LANG} {IT}, {IT}, {LANG} {IT}\.$",
        fr"{LANG} {IT}, {IT}, and prob\. unacc\. {IT}, {IT}\.$",
        fr"{LANG} {IT}, {IT}, distinct from {IT}\.$",
        fr"{LANG} {IT}, {IT}, occas\. {IT}\.$",
        fr"{LANG} {IT}, {IT}, str\. and wk\.$",
        fr"{LANG} {IT}, {IT}, str\., later wk\.$",
        fr"{LANG} {IT}, {IT}; {GRM} {IT}; {LANG} {GRM} {IT}\.$",
        fr"{LANG} {IT}, {IT}; {GRMS} {IT}, {IT}\.$",
        fr"{LANG} {IT}, {IT}; {IT}, {IT}, {GRM}$",
        fr"{LANG} {IT}, {IT}; {IT}, {IT}\.$",
        fr"{LANG} {IT}, {IT}; {IT}; {IT}\.$",
        fr"{LANG} {IT}, {IT}; {IT}\.$",
        fr"{LANG} {IT}, {IT}; {LANG} {IT}\.$",
        fr"{LANG} {IT}, {IT}; cf\. {LANG} {IT}\.$",
        fr"{LANG} {IT}, {IT}; later {IT}\.$",
        fr"{LANG} {IT}, {IT}; pp\. {IT}\.$",
        fr"{LANG} {IT}, {IT}\.$",
        fr"{LANG} {IT}, {LANG} {IT}, {ETC}, pp\.$",
        fr"{LANG} {IT}, {LANG} {IT}, {GLOSS}\.$",
        fr"{LANG} {IT}, {LANG} {IT}, {GRM}$",
        fr"{LANG} {IT}, {LANG} {IT}; see {NED}$",
        fr"{LANG} {IT}, {LANG} {IT}\.$",
        fr"{LANG} {IT}, \? {INFL_BY} {IT}, {ETC}",
        fr"{LANG} {IT}, \? {INFL_BY} {IT}\.$",
        fr"{LANG} {IT}, \? {IT}\.$",
        fr"{LANG} {IT}, \({GRMS}\) {GLOSS}\.$",
        fr"{LANG} {IT}, \(late\) {IT}\.$",
        fr"{LANG} {IT}, \(once in gloss\.\) {IT}\.$",
        fr"{LANG} {IT}, \(once\) {IT}\.$",
        fr"{LANG} {IT}, and {IT} prefix\.$",
        fr"{LANG} {IT}, and \(from 14th c\.\) {IT}, {ETC}$",
        fr"{LANG} {IT}, and occas\. in same sense {IT}\.$",
        fr"{LANG} {IT}, and with weak stress {IT}\(\?\)\.$",
        fr"{LANG} {IT}, cf\. {IT}\.$",
        fr"{LANG} {IT}, corruptly from {LANG} {IT}\.$",
        fr"{LANG} {IT}, earlier {IT}\.$",
        fr"{LANG} {IT}, early confused with {IT}\.$",
        fr"{LANG} {IT}, expose, be exposed, to the weather\.$",
        fr"{LANG} {IT}, formed on {LANG} {IT}, pp\.$",
        fr"{LANG} {IT}, from {CAP_WORD}, {QV}$",
        fr"{LANG} {IT}, from {IT}, {GLOSS}\.$",
        fr"{LANG} {IT}, from {LANG} {IT}\.$",
        fr"{LANG} {IT}, from {LANG}$",
        fr"{LANG} {IT}, in {IT}, {GLOSS}\.$",
        fr"{LANG} {IT}, in {IT}; see {NED}, s.v. _{CAP_WORD}_\.$",
        fr"{LANG} {IT}, infl\. by {CAP_WORD}\.$",
        fr"{LANG} {IT}, infl\. by related verb; _see_ {CAP_WORD}\.$",
        fr"{LANG} {IT}, infl\. by suffix {IT}\.$",
        fr"{LANG} {IT}, infl\. in sense by cognate {LANG} {IT}\.$",
        fr"{LANG} {IT}, influ\. in {LANG} by assoc\. with {IT}\.$",
        fr"{LANG} {IT}, influenced by {IT}\.$",
        fr"{LANG} {IT}, late {GRMS} {IT}\.$",
        fr"{LANG} {IT}, late {IT}\.$",
        fr"{LANG} {IT}, later {IT}\.$",
        fr"{LANG} {IT}, oblique forms of {IT}\.$",
        fr"{LANG} {IT}, older {IT}\.$",
        fr"{LANG} {IT}, older, {IT}\.$",
        fr"{LANG} {IT}, or {IT}; {LANG} {IT}\.$",
        fr"{LANG} {IT}, or {IT}\.$",
        fr"{LANG} {IT}, or {LANG} {IT}, {IT}\.$",
        fr"{LANG} {IT}, or {LANG} {IT}\.$",
        fr"{LANG} {IT}, orig\. 'mated' in chess\.$",
        fr"{LANG} {IT}, pers\. and impers\.$",
        fr"{LANG} {IT}, pp\. {IT}; cf\. {LANG} pp\. {IT}\.$",
        fr"{LANG} {IT}, pp\. {IT}\.$",
        fr"{LANG} {IT}, pp\. {IT}$",  # @@@ missing final .
        fr"{LANG} {IT}, pp\., {GLOSS}\.$",
        fr"{LANG} {IT}, prob\. infl\. by senses of {LANG} {IT}\.$",
        fr"{LANG} {IT}, reduced under wk\. stress\.$",
        fr"{LANG} {IT}, rel\. to {CAP_WORD}, _{GRM}_{SUP}$",
        fr"{LANG} {IT}, rel\. to {CAP_WORD}, _{GRM}_$",
        fr"{LANG} {IT}, rel\. to {IT}, {CAP_WORD}\.$",
        fr"{LANG} {IT}, rel\. to {IT}, {GLOSS}\.$",
        fr"{LANG} {IT}, stem of {IT}\.$",
        fr"{LANG} {IT}, str\.; {IT}, wk\.; both intr\.$",
        fr"{LANG} {IT}, to file; or {LANG} {IT}\.$",
        fr"{LANG} {IT}, with {IT} > {IT}\.$",
        fr"{LANG} {IT}, with {LANG} {IT} > {IT}\.$",
        fr"{LANG} {IT}; _cf\._ next\.$",
        fr"{LANG} {IT}; _see_ {CAP_WORD}\.$",
        fr"{LANG} {IT}; _see_ next\.$",
        fr"{LANG} {IT}; _see_ prec\.$",
        fr"{LANG} {IT}; {GRM} {IT}, {IT}\.$",
        fr"{LANG} {IT}; {GRMS} {IT} \({LANG} {IT}\)\.$",
        fr"{LANG} {IT}; {GRMS} {IT}, {IT}\.$",
        fr"{LANG} {IT}; {GRMS} {IT}\.$",
        fr"{LANG} {IT}; {IT} {GRMS}; {IT}, {ETC}$",
        fr"{LANG} {IT}; {IT} \({GRMS}\) \+ {IT}\.$",
        fr"{LANG} {IT}; {IT} = {IT} \(see the rimes\)\.$",
        fr"{LANG} {IT}; {IT} is freq\. Northern form\.$",
        fr"{LANG} {IT}; {IT}, _{GRM}_$",
        fr"{LANG} {IT}; {IT}, {GRM}$",
        fr"{LANG} {IT}; {IT}, {IT} {GRM}$",
        fr"{LANG} {IT}; {IT}, {IT}; {IT}, {ETC}$",
        fr"{LANG} {IT}; {IT}, {IT}\.$",
        fr"{LANG} {IT}; {IT}, pp\.$",
        fr"{LANG} {IT}; {IT}\.$",
        fr"{LANG} {IT}; {LANG} {IT}, {GLOSS}\. _Cf\._ {CAP_WORD}, _{GRM}_$",
        fr"{LANG} {IT}; {LANG} {IT}, {GLOSS}\.$",
        fr"{LANG} {IT}; {LANG} {IT}, {GRM}$",
        fr"{LANG} {IT}; {LANG} {IT}, {IT}\.$",
        fr"{LANG} {IT}; {LANG} {IT}\.$",
        fr"{LANG} {IT}; {LANG} vowel and usage show infl\. of {CAP_WORD}\.$",
        fr"{LANG} {IT}; {LANG}, {LANG} {IT}\.$",
        fr"{LANG} {IT}; \? {INFL_BY} {IT}\.$",
        fr"{LANG} {IT}; \? {LANG} {IT} \(in sense '{GLOSS}'\)\.$",
        fr"{LANG} {IT}; \? from {LANG} {IT}, {GLOSS}, _{GRM}_$",
        fr"{LANG} {IT}; accented stem {IT}, {IT}, {ETC}$",
        fr"{LANG} {IT}; and direct from {LANG} {IT}\.$",
        fr"{LANG} {IT}; but see {NED}$",
        fr"{LANG} {IT}; cf\. {CAP_WORD}, _{GRM}_{SUP}$",
        fr"{LANG} {IT}; cf\. {IT}, {LANG} {IT}\.$",
        fr"{LANG} {IT}; cf\. {IT}, and {LANG} {IT}\.$",
        fr"{LANG} {IT}; cf\. {IT}, and next\.$",
        fr"{LANG} {IT}; cf\. {IT}\.$",
        fr"{LANG} {IT}; cf\. {LANG} {IT} {GLOSS}\.$",
        fr"{LANG} {IT}; cf\. {LANG} {IT}, {IT}, in this sense\.$",
        fr"{LANG} {IT}; cf\. {LANG} {IT}, {LANG} {IT}\.$",
        fr"{LANG} {IT}; cf\. {LANG} {IT}\.$",
        fr"{LANG} {IT}; cf\. next\.$",
        fr"{LANG} {IT}; early {LANG} {IT}\.$",
        fr"{LANG} {IT}; obscure\.$",
        fr"{LANG} {IT}; on {IT}-form see {NED} s\.v\. _{CAP_WORD}_\.$",
        fr"{LANG} {IT}; on stem-vowel see {NED} s\.v\. _{CAP_WORD}_\.$",
        fr"{LANG} {IT}; on the vowel _see_ {CAP_WORD}\.$",
        fr"{LANG} {IT}; orig\. same word as {CAP_WORD}, _{GRM}_$",
        fr"{LANG} {IT}; see {NED}, s\.v\. _{CAP_WORD}_\.$",
        fr"{LANG} {IT}; see {NED}, s\.v\. {IT}, {GRM}$",
        fr"{LANG} {IT}; see {NED}$",
        fr"{LANG} {IT}; see next\.$",
        fr"{LANG} {IT}; see note\.$",
        fr"{LANG} {IT}; see prec\.$",
        fr"{LANG} {IT}; the {IT} forms are difficult to explain.$",
        fr"{LANG} {IT}; the confusion with {IT} began in {LANG}$",
        fr"{LANG} {IT}; with {IT} cf\. {CAP_WORD}, {CAP_WORD}\.$",
        fr"{LANG} {IT}; with the reduced forms cf\. {CAP_WORD}\.$",
        fr"{LANG} {IT}. On {IT} for {IT}, _see_ App\., p\. 278\.$",
        fr"{LANG} {IT}{SUP} \+ {IT}\.$",
        fr"{LANG} {IT}\. \(cf\. {LANG} {IT}\)\.$",
        fr"{LANG} {IT}\.{SUP}$",
        fr"{LANG} {IT}\.$",
        fr"{LANG} \? {IT}, rel\. to {IT}\.$",
        fr"{LANG} \? {IT}; {LANG} {IT}\.$",
        fr"{LANG} \(allit\.\) {IT}, {GLOSS}\.$",
        fr"{LANG} \(from {LANG}\) {IT}\.$",
        fr"{LANG} \(i\) {IT}, \(ii\) {IT}\.$",
        fr"{LANG} \(late\) {IT}, prob\. modelled on {LANG} {IT}\.$",
        fr"{LANG} \(once\) {IT}; cf\. {IT}, {GRM}$",
        fr"{LANG} \(only {LANG}\) {IT}, of unknown origin\.$",
        fr"{LANG} \(rare\) {IT}\.$",
        fr"{LANG} also {IT}; \? obscurely rel\. to {CAP_WORD}, {QV}$",
        fr"{LANG} forms point to {LANG} {IT}\.$",
        fr"{LANG} in {IT}, {IT}\.$",
        fr"{LANG} in {IT}; see {NED} s\.v\. {IT}\.$",
        fr"{LANG} in {IT}\.$",
        fr"{LANG} unacc\. form {IT}, or {LANG} {IT}; _see_ {CAP_WORD}, {CAP_WORD}\.$",
        fr"{LANG}, {LANG} {IT}, {LANG} {IT}\.$",
        fr"{LANG}, {LANG} {IT}\.$",
        fr"{LANG}; also appears in {LANG} in {LANG} form {IT}\.$",
        fr"{LANG}$",
        fr"\? {LANG} {IT} \(cf\. {IT}\)\.$",
        fr"\? {LANG} {IT} \+ {IT}; cf\. {IT}, Layamon 969\.$",
        fr"\? {LANG} {IT} < {IT} \(cf. {LANG} {IT}\)\.$",
        fr"\? {LANG} {IT}, {GLOSS}; _cf\._ {CAP_WORD}\.$",
        fr"\? {LANG} {IT}, {GLOSS}; cf\. senses of {IT}.$",
        fr"\? {LANG} {IT}, {GRMS}, or {GRM}, of {IT}\.$",
        fr"\? {LANG} {IT}, {IT}, {GLOSS}, {GLOSS}; cf\. {CAP_WORD}\.$",
        fr"\? {LANG} {IT}, {IT}, {GLOSS}\.$",
        fr"\? {LANG} {IT}; cf\. {LANG} {IT}\.$",
        fr"\? {LANG} {IT}; earliest {LANG} sense appar\. '{GLOSS}'.",
        fr"\? {LANG} {IT}; see {NED}$",
        fr"\? {LANG} {IT}\.$",
        fr"\? Altered form of {LANG} {IT}\.$",
        fr"\? Cf\. {LANG} {IT}, {GLOSS}\.$",
        fr"\? From next\.$",
        fr"\? From prec\.$",
        fr"\? Obscure alteration of {LANG} {IT}\.$",
        fr"\? Rel\. to next\.$",
        fr"\? Related \(as {IT} to {IT}\) to {LANG} {IT}\.$",
        fr"\? Related to {CAP_WORD}, _{GRM}_{SUP}$",
        fr"\? Related to {CAP_WORD}, _{GRM}_$",
        fr"\? Same as next\.$",
        fr"A Northern form\. ON\. {IT}\.$",
        fr"A variant, usually Northern, of {CAP_WORD}, {QV}$",
        fr"Altered by assoc\. with prec\. from {LANG} {IT}\.$",
        fr"As {CAP_WORD}, with alteration of final spirant; _cf\._ {CAP_WORD}\.$",
        fr"As next with subst\. of interchangeable {IT}\.$",
        fr"As next\.$",
        fr"As prec\.; for older {GRMS} _see_ {CAP_WORD}\.$",
        fr"As prec\.$",
        fr"Back-formation from {CAP_WORD}\.$",
        fr"Blend of {LANG} {IT}, {GLOSS}, and {LANG} {IT}, {GLOSS}\.$",
        fr"Blend of {LANG} {IT}, and {LANG} {IT}\.$",
        fr"Cf. {LANG} gloss {IT}, scopulus; {LANG} {IT}\.$",
        fr"Cf\. {LANG} _[^_]+!_$",
        fr"Cf\. {LANG} {IT}, {GLOSS}, and _{CAP_WORD}_{SUP} in _E\.D\.D\._$",
        fr"Cf\. {LANG} {IT}, {GLOSS}\.$",
        fr"Cf\. {LANG} {IT}, {GRM}, {GLOSS}\.$",
        fr"Cf\. {LANG} {IT}, {GRM}$",
        fr"Cf\. {LANG} {IT}, {IT}\.$",
        fr"Cf\. {LANG} {IT}, {LANG} {IT}, {GLOSS}\.$",
        fr"Cf\. {LANG} {IT}, {LANG} {IT}\.$",  # , vs ;
        fr"Cf\. {LANG} {IT}, {LANG}, {LANG} {IT}\.$",
        fr"Cf\. {LANG} {IT}, w\(h\)op, {GLOSS}; echoic\.$",  # @@@
        fr"Cf\. {LANG} {IT}; {LANG} {IT}\.$",  # , vs ;
        fr"Cf\. {LANG} {IT}; \? {LANG} {IT}\.$",
        fr"Cf\. {LANG} {IT}; see {NED}$",
        fr"Cf\. {LANG} {IT}.$",
        fr"Cf\. {LANG}, {LANG} {IT}, v\.$",
        fr"Cf\. {LANG}, {LANG}, {IT}\.$",
        fr"Cf\. MDu\., MLG\. {IT}\.$",
        fr"Children's language\.$",
        fr"Echoic, on model of next\.",
        fr"Echoic; _cf\._ {CAP_WORD}, {CAP_WORD}\.$",
        fr"Echoic; _cf\._ {CAP_WORD}\.$",
        fr"Echoic; cf\. {LANG} {IT}; {LANG} {IT}, {GRM}$",
        fr"Echoic\.$",
        fr"Extended from {CAP_WORD} with abstract {IT}\.$",
        fr"Extended from {CAP_WORD}, _conj\._, with {GRM} {IT}\.$",
        fr"Extended from {LANG} {IT}; _cf\._ {CAP_WORD}, {CAP_WORD}\.$",
        fr"Extended from {LANG} {IT}\.$",
        fr"Extended from prec\.$",
        fr"First two words of Latin prayer\.$",
        fr"Formed on {CAP_WORD} _{GRM}_$",  # inconsistenly missing commas
        fr"Formed on {CAP_WORD}, _{GRM}_$",
        fr"Formed on {LANG} {IT} pp\. of {IT}\.$",
        fr"From _prec\._$",
        fr"From {CAP_WORD}, _{GRM}_; cf\. {LANG} {IT}\.$",
        fr"From {CAP_WORD}, _{GRM}_{SUP}$",
        fr"From {CAP_WORD}, _{GRM}_$",
        fr"From {CAP_WORD}, {CAP_WORD}\.$",
        fr"From {CAP_WORD}, {QV}$",
        fr"From {CAP_WORD}, to buy\. _See_ {CAP_WORD}, _{GRM}_; {CAP_WORD}\.$",
        fr"From {CAP_WORD}\.$",
        fr"From {IT}, {GLOSS}; cf\. {LANG} {IT}\.$",
        fr"From {IT}, old infin\. stem of {CAP_WORD}, _{GRM}_{SUP}$",
        fr"From {IT}\.$",
        fr"From {LANG} {IT} \+ {IT}\.$",
        fr"From {LANG} {IT}, {ETC}, extended from prec\.$",
        fr"From {LANG} {IT}, {GLOSS}; obscure\.$",
        fr"From {LANG} {IT}, {GLOSS}\.$",  # inconsistent comma?
        fr"From {LANG} {IT}, {GRM}$",
        fr"From {LANG} {IT}, {IT} \({LANG} {IT}\), {GLOSS}\.$",
        fr"From {LANG} {IT}, {IT}, {GRM}; _see_ {CAP_WORD}\.$",
        fr"From {LANG} {IT}, {IT}, {GRM}; {LANG} {IT}\.$",
        fr"From {LANG} {IT}, {IT}, {GRM}$",
        fr"From {LANG} {IT}, {IT}, on anal\. of {IT}, {IT}\.$",
        fr"From {LANG} {IT}, {IT}, pp\.$",
        fr"From {LANG} {IT}, {IT}\.$",
        fr"From {LANG} {IT}, {LANG} {IT} \(related to prec\.\)\.$",
        fr"From {LANG} {IT}, {LANG} {IT}, {GLOSS}\.$",
        fr"From {LANG} {IT}, {LANG} {IT}\.$",
        fr"From {LANG} {IT}, extended from {CAP_WORD}, _{GRM}_{SUP}$",
        fr"From {LANG} {IT}, formed on {CAP_WORD}, {QV}$",
        fr"From {LANG} {IT}, to lounge; _see_ Piers Pl\. C X 215\.$",
        fr"From {LANG} {IT}; cf\. {LANG} {IT}\.$",
        fr"From {LANG} {IT}\.$",
        fr"From {LANG}, {LANG} {IT}, {GRM}$",
        fr"From {LANG}, {LANG} {IT}\.$",
        fr"From \(obscure\) {LANG} {IT}, {GLOSS}; see {NED}$",
        fr"From next; cf, {LANG} {IT}\.$",  # @@@
        fr"From next\.$",
        fr"From pp\. of {CAP_WORD}, {QV}$",
        fr"From prec\. \(cf\. {REF}\); cf\. {LANG} {IT}\.$",
        fr"From prec\. \(i\)\.$",
        fr"From prec\. in {LANG} sense '{GLOSS}'\.$",
        fr"From prec\. in frequent sense '{GLOSS}'\.$",
        fr"From prec\.; _cf\._ {LANG} {IT}, {IT}\.$",
        fr"From prec\.; _cf\._ Cath\. Angl\., '_coker_, autumnarius'\.$",  # @@@
        fr"From prec\.; {LANG} {IT}\.$",
        fr"From prec\.; cf\. {LANG} {IT}, {GRM}$",
        fr"From prec\.; cf\. {LANG} {IT}\.$",
        fr"From prec\.$",
        fr"From pres\. p\. of {CAP_WORD}\.$",
        fr"From stem of {CAP_WORD}; cf\. {LANG} {IT}\.$",
        fr"Further reduced from {CAP_WORD}, {QV}$",
        fr"Late {LANG} {IT}, from {LANG} {IT}, {LANG} {IT}\.$",
        fr"Late {LANG} {IT}, from {LANG} {IT}\.$",
        fr"Late OE\. {IT} from {LANG} {IT}\.$",
        fr"Late OE\. {IT}, {IT}\.",
        fr"Modelled on {LANG} {IT}\.$",
        fr"Next \+ {LANG} {IT}\.$",
        fr"Nonce-use of {LANG} {IT}, {GLOSS}\.$",
        fr"Northern form of {CAP_WORD}, {QV}$",
        fr"Not known; only allit\.$",
        fr"Not known\.$",
        fr"Obscure; \? cf\. {CAP_WORD}, and {LANG} {IT}\.$",
        fr"Obscure; \? cf\. {LANG} {IT}, {GLOSS}\.$",
        fr"Obscure; appar\. peculiar to {CAP_WORD}\.$",
        fr"Obscure; cf\. {LANG}, {LANG} {IT}\.$",
        fr"Obscure; see {NED}, s\.v\. _{CAP_WORD}_\.$",
        fr"Obscure; usually Northern\.$",
        fr"Obscure\.$",
        fr"Obscurely rel\. to {LANG} {IT}, build; see {NED}$",
        fr"OE\. {IT} \({IT}, but not WS\.\)\.$",
        fr"OE\. {IT} \(Kt\. {IT}\); {IT}, {IT}\.$",
        fr"OE\. {IT} \(Kt\. {IT}\)\.$",  # Kt inconsistency
        fr"OE\. {IT} \(late WS\. {IT}\)\.$",
        fr"OE\. {IT} \(rare\)\.$",
        fr"OE\. {IT}, \(Kt\. {IT}\)\.$",  # Kt inconsistency
        fr"OE\. {IT}, \(Kt\.\) {IT}\.$",  # Kt inconsistency
        fr"OE\. {IT}, Kt\. {IT}\.$",  # Kt inconsistency
        fr"OE\. {IT}, late {LANG} {IT}\.$",
        fr"OE\. {IT}; {GRM} {IT} \(Kt\. {IT}\)\.$",
        fr"OE\. \({LANG}\) {IT} from {LANG} {IT}\.$",
        fr"OE\. \({LANG}\) {IT}\.$",
        fr"OE\. \(late 11th c\.\) {IT}\.$",
        fr"OE\. \(late\) {IT} from {LANG} {IT}\.$",
        fr"OE\. \(late\) {IT}, {GLOSS}\.$",
        fr"OE\. \(late\) {IT}, from {LANG} {IT}\.$",
        fr"OE\. \(late\) {IT}\.$",
        fr"OE\. \(rare {LANG}\) {IT}; {LANG} {IT}\.$",
        fr"OE\. \(WS\.\) {IT}\.$",
        fr"Origin of name doubtful; see {NED}$",
        fr"Perh\. distinct verbs; see {NED}, s.v. _{CAP_WORD}_.$",
        fr"Pp\. of {CAP_WORD}.$",
        fr"Prec\. \+ _pp\._ of {CAP_WORD}\.$",
        fr"Prec\. \+ {CAP_WORD}, {QV}$",
        fr"Prec\. \+ {CAP_WORD}\.$",
        fr"Prec\. \+ {GRM} {IT}\.$",
        fr"Prec\. \+ {IT}\.$",
        fr"Prec\. \+ {LANG} {IT}, {IT}\.$",
        fr"Prec\. \+ {LANG} {IT}; {LANG} {IT}\.$",
        fr"Prec\. \+ {LANG} {IT}\.$",
        fr"Prec\. and {LANG} {IT}, border\.$",
        fr"Prob\. {LANG} {IT}, rel\. to prec\.$",
        fr"Prob\. from prec\.$",
        fr"Prob\. rel\. to {LANG} {IT}\.$",
        fr"Prob\. related to {CAP_WORD}\.$",
        fr"Prob\. same as {LANG} {IT}, {GLOSS}, {GLOSS}, {ETC}$",
        fr"Prob\. same as prec\.$",
        fr"Reduced form of {CAP_WORD}, _{GRM}_$",
        fr"Reduced form of {CAP_WORD}, {QV}$",
        fr"Reduced unaccented form of {IT}; _see_ {CAP_WORD}\.$",
        fr"Reduction of {CAP_WORD}{SUP}, {QV}$",
        fr"Rel\. to {CAP_WORD}, _{GRM}_{SUP}; cf\. {LANG}, {LANG} {IT}\.$",
        fr"Related to {CAP_WORD} as prec\.$",
        fr"See {NED} s\.v\. {IT}\.$",
        fr"See {NED} s\.vv\. {IT}, {GRM}{SUP}, {IT}.$",
        fr"See {NED} s\.vv\. {IT}, {IT}\.$",
        fr"See {NED}, s\.v\. {IT} \d+\.$",  # not quite sure what the last part means
        fr"See {NED}, s\.v\. {IT}, and {IT}\.$",
        fr"See {NED}, s\.v\. {IT}\.$",  # inconsistent comma
        fr"See {NED}$",
        fr"Shortened from {CAP_WORD}, {CAP_WORD}\.$",
        fr"Shortened from {CAP_WORD}, {QV}$",
        fr"Shortened from {CAP_WORD}\.$",
        fr"Shortened from {IT}, {LANG} {IT}\.$",
        fr"Shortened from {LANG} {IT}; _see_ {CAP_WORD}\.$",
        fr"Shortened from {LANG} {IT}\.$",
        fr"Stem of {CAP_WORD} _{GRM}_ \+ {LANG} {IT}\.$",
        fr"Stem of {CAP_WORD} \+ {LANG} {IT}; cf\. {LANG} {IT}\.$",
        fr"Stem of {CAP_WORD} with {LANG} monophthongization\.$",
        fr"Stem of {LANG} {IT} \+ {IT}\.",
        fr"Stem of {LANG} {IT}, {GRM}, {LANG} {IT}\.$",
        fr"Stem of {LANG} {IT}, {GRM}$",
        fr"Stem of {LANG} {IT}, or {LANG} {IT}; _see_ {CAP_WORD}\.$",
        fr"Stem of {LANG} {IT}\.$",
        fr"Stem of next\.$",
        fr"Unaccented form of {LANG} {IT}\. _See_ {CAP_WORD}, {CAP_WORD}\.$",
        fr"Unaccented reduction of {CAP_WORD}\.$",
        fr"Unknown; _cf\._ {LANG} {IT}\.$",
        fr"Unknown\.$",
    ]
]


def check_etymology(etymology):
    for regex in regexes:
        if regex.match(etymology):
            return True
    return False
